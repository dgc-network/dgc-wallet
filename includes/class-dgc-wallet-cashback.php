...
if (!class_exists('dgc_Wallet_Cashback')) {
    ...
    class dgc_Wallet_Cashback {
        ...
        private static function init_cashback_settings() {
            self::$cashback_rule = dgc_wallet()->settings_api->get_option('cashback_rule', '_wallet_settings_credit', 'cart');
            self::$global_cashbak_type = dgc_wallet()->settings_api->get_option('cashback_type', '_wallet_settings_credit', 'percent');
            self::$global_cashbak_amount = floatval(dgc_wallet()->settings_api->get_option('cashback_amount', '_wallet_settings_credit', 0));
            self::$max_cashbak_amount = floatval(dgc_wallet()->settings_api->get_option('max_cashback_amount', '_wallet_settings_credit', 0));
        }
        ...
        public static function calculate_cashback($form_cart = true, $order_id = 0, $force = false) {
            ...
            if ('on' != dgc_wallet()->settings_api->get_option('is_enable_cashback_reward_program', '_wallet_settings_credit')) {
                return 0;
            }
        ...
        private static function calculate_cashback_form_cart() {
            ...
            switch (self::$cashback_rule) {
                ...
                case 'product_cat':
                    ...
                    break;
                case 'cart':
                    if (dgc_wallet()->settings_api->get_option('min_cart_amount', '_wallet_settings_credit', 10) != 0 && WC()->cart->get_subtotal('edit') >= dgc_wallet()->settings_api->get_option('min_cart_amount', '_wallet_settings_credit', 0)) {
            ...
            return apply_filters('dgc_wallet_form_cart_cashback_amount', $cashback_amount);
        }
        ...
        private static function calculate_cashback_form_order($order_id = 0) {
            ...
            switch (self::$cashback_rule) {
                ...
                case 'product_cat':
                    ...
                    break;
                case 'cart':
                    if (dgc_wallet()->settings_api->get_option('min_cart_amount', '_wallet_settings_credit', 10) != 0 && $order->get_subtotal('edit') >= dgc_wallet()->settings_api->get_option('min_cart_amount', '_wallet_settings_credit', 0)) {
                        if ('percent' === self::$global_cashbak_type) {
            ...
            $cashback_amount = apply_filters('dgc_wallet_form_order_cashback_amount', $cashback_amount, $order_id);
        ...
        public static function get_product_cashback_amount($product, $qty = 1) {
            ...
            return apply_filters('dgc_wallet_product_wise_cashback_amount', $cashback_amount, $product, $qty);
        }
        ...
        public static function get_product_category_wise_cashback_amount($product, $qty = 1) {
            ...
            if (!empty($category_wise_cashback_amounts)) {
                $cashback_amount += ( 'on' === dgc_wallet()->settings_api->get_option('allow_min_cashback', '_wallet_settings_credit', 'on') ) ? min($category_wise_cashback_amounts) : max($category_wise_cashback_amounts);
            } else {
                if ('percent' === self::$global_cashbak_type) {
            ...
            return apply_filters('dgc_wallet_product_category_wise_cashback_amount', $cashback_amount, $product, $qty);
        }
