...
class Action_Product_Review extends dgc_WalletAction {

    public function __construct() {
        $this->id = 'product_review';
        $this->action_title = __( 'WooCommerce product review', 'dgc-wallet' );
        $this->description = __( 'Set credit for WooCommerce product review', 'dgc-wallet' );
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type'    => 'checkbox',
                'label'   => __( 'Enable credit for WooCommerce product review', 'dgc-wallet' ),
                'default' => 'no',
            ),
            'amount' => array(
                'title'       => __( 'Amount', 'dgc-wallet' ),
                'type'        => 'price',
                'description' => __( 'Enter amount which will be credited to the user wallet for reviewing a WooCommerce product.', 'dgc-wallet' ),
                'default'     => '10',
                'desc_tip'    => true
            ),
            'description' => array(
                ...
                'default'     => __( 'Balance credited for reviewing a WooCommerce product.', 'dgc-wallet' ),
                'desc_tip'    => true,
            )
        );
    }
    ...
    public function new_product_review( $comment_ID, $comment_approved, $commentdata ) {
        if ( 'product' === get_post_type(absint( $commentdata['comment_post_ID'] ) ) ) { // WPCS: input var ok, CSRF ok.
            ...
            if ( $this->settings['amount'] && $product && apply_filters( 'dgc_wallet_product_review_credit', true, $commentdata ) ) {
                $transaction_id = dgc_wallet()->wallet->credit( $commentdata['user_id'], $this->settings['amount'], sanitize_textarea_field( $this->settings['description'] ) );
    ...
    public function dgc_wallet_product_review_credit( $new_status, $old_status, $comment ) {
        ...
        if ( $this->settings['amount'] && $product && apply_filters( 'dgc_wallet_product_review_credit', true, $comment ) ) {
            $transaction_id = dgc_wallet()->wallet->credit( $comment->user_id, $this->settings['amount'], sanitize_textarea_field( $this->settings['description'] ) );
