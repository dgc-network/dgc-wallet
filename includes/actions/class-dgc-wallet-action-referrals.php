...
class Action_Referrals extends dgc_wallet_Action {
    ...
    public function add_referral_nav_menu($nav_menu, $is_rendred_from_myaccount) {
        $nav_menu['referrals'] = array(
            'title' => apply_filters('dgc_wallet_account_referrals_menu_title', __('Referrals', 'text-domain')),
            'url' => $is_rendred_from_myaccount ? esc_url(wc_get_endpoint_url(get_option('woocommerce_dgc_wallet_endpoint', 'text-domain'), 'referrals', wc_get_page_permalink('myaccount'))) : add_query_arg('payment_action', 'referrals', get_permalink()),
    ...
    public function referral_content() {
        global $wp;
        if (apply_filters('dgc_wallet_is_enable_referrals', true) && ( ( isset($wp->query_vars['dgc-wallet']) && 'referrals' === $wp->query_vars['dgc-wallet'] ) || ( isset($_GET['payment_action']) && 'referrals' === $_GET['payment_action'] ) )) {
