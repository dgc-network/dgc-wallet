...
class dgc_Wallet_Balance_Details extends WP_List_Table {
    ...
    public function get_columns() {
        return apply_filters( 'dgc_wallet_balance_details_columns', array(
            'id'       => __( 'ID', 'dgc-wallet' ),
            'username' => __( 'Username', 'dgc-wallet' ),
            'name'     => __( 'Name', 'dgc-wallet' ),
            'email'    => __( 'Email', 'dgc-wallet' ),
            'balance'  => __( 'Remaining balance', 'dgc-wallet' ),
            'actions'  => __( 'Actions', 'dgc-wallet' ),
        ) );
    }
    ...
    public function prepare_items() {
        ...
        $args = apply_filters( 'dgc_wallet_users_list_table_query_args', $args );

        // Query the user IDs for this page
        $wp_user_search = new WP_User_Query( $args );
        $data = array();
        foreach ( $wp_user_search->get_results() as $user) {
            $data[] = apply_filters('dgc_wallet_balance_details_list_table_item_data', array(
                'id'       => $user->ID,
                'username' => $user->data->user_login,
                'name'     => $user->data->display_name,
                'email'    => $user->data->user_email,
                'balance'  => dgc_wallet()->wallet->get_wallet_balance( $user->ID ),
                'actions'  => ''
            ), $user);
        }
    ...
    */
    ...
    public function get_sortable_columns() {
        ...
        return apply_filters( 'dgc_wallet_balance_details_sortable_columns', $sortable_columns );
    }
    ...
    public function column_default( $item, $column_name ) {
        switch ( $column_name ) {
            ...
            case 'actions':
                return '<p><a href="' . add_query_arg( array( 'page' => 'dgc-wallet-add', 'user_id' => $item['id'] ), admin_url( 'admin.php' ) ) . '" class="button tips wallet-manage"></a> <a class="button tips wallet-view" href="' . add_query_arg( array( 'page' => 'dgc-wallet-transactions', 'user_id' => $item['id'] ), admin_url( 'admin.php' ) ) . '"></a></p>';
            default:
                return apply_filters('dgc_wallet_balance_details_column_default', print_r( $item, true ), $column_name, $item);
        }
    }
