...
class dgc_Payment_Balance_Details extends WP_List_Table {
    ...
    public function prepare_items() {
            ...
            if ('balance' === $args['orderby']) {
                $args = array_merge(
                        $args, array(
                            ...
                            'meta_key' => '_current_dgc_payment_balance',
        ...
        $args = apply_filters( 'dgc_payment_users_list_table_query_args', $args );

        // Query the user IDs for this page
        $wp_user_search = new WP_User_Query( $args );
        $data = array();
        foreach ( $wp_user_search->get_results() as $user) {
            $data[] = apply_filters('dgc_payment_balance_details_list_table_item_data', array(
                'id'       => $user->ID,
                'username' => $user->data->user_login,
                'name'     => $user->data->display_name,
                'email'    => $user->data->user_email,
                'balance'  => dgc_payment()->payment->get_payment_balance( $user->ID ),
                'actions'  => ''
            ), $user);
        }
    ...
    */
    protected function get_views() {
        ...
        $url           = 'admin.php?page=dgc-payment';
    ...
    */
    protected function extra_tablenav( $which ) {
        do_action('dgc_payment_users_list_extra_tablenav', $which);
    }
    ...
    public function get_sortable_columns() {
        ...
        return apply_filters( 'dgc_payment_balance_details_sortable_columns', $sortable_columns );
    }
    ...
    public function column_default( $item, $column_name ) {
        switch ( $column_name ) {
            ...
            case 'actions':
                return '<p><a href="' . add_query_arg( array( 'page' => 'dgc-payment-add', 'user_id' => $item['id'] ), admin_url( 'admin.php' ) ) . '" class="button tips payment-manage"></a> <a class="button tips payment-view" href="' . add_query_arg( array( 'page' => 'dgc-payment-transactions', 'user_id' => $item['id'] ), admin_url( 'admin.php' ) ) . '"></a></p>';
            default:
                return apply_filters('dgc_payment_balance_details_column_default', print_r( $item, true ), $column_name, $item);
        }
    }
