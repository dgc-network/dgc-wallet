...
if ( ! class_exists( 'dgc_Wallet_Ajax' ) ) {
    ...
    class dgc_Wallet_Ajax {
        ...
        public function dgc_wallet_refund_partial_payment(){
            ...
            $transaction_id = dgc_wallet()->wallet->credit( $order->get_customer_id(), $partial_payment_amount, __( 'Wallet refund #', 'dgc-wallet' ) . $order->get_order_number() );
            if($transaction_id){
                $response['success'] = true;
                $order->add_order_note(sprintf( __( '%s refunded to customer wallet', 'dgc-wallet' ), wc_price( $partial_payment_amount, dgc_wallet_wc_price_args($order->get_customer_id()) ) ));
        ...
        public function dgc_wallet_order_refund() {
            ...
            try {
                ...
                if ( !$refund_amount || $max_refund < $refund_amount || 0 > $refund_amount ) {
                    throw new exception( __( 'Invalid refund amount', 'dgc-wallet' ) );
                }
                ...
                if ( ! is_wp_error( $refund ) ) {
                    $transaction_id = dgc_wallet()->wallet->credit( $order->get_customer_id(), $refund_amount, __( 'Wallet refund #', 'dgc-wallet' ) . $order->get_order_number() );
                    if ( !$transaction_id ) {
            ...
            if ( apply_filters( 'dgc_wallet_user_search_exact_match', true ) ) {
                ...
                if ( $user && wp_get_current_user()->user_email != $user->user_email) {
                    $return[] = array(
                        ...
                        'label' => sprintf(_x( '%1$s (%2$s)', 'user autocomplete result', 'dgc-wallet' ), $user->user_login, $user->user_email),
                        'value' => $user->ID,
                    );
                }
            } else {
        ...
        public function dgc_wallet_dismiss_promotional_notice(){
            ...
            if ( ! current_user_can( 'manage_options' ) ) {
                wp_send_json_error( __( 'You have no permission to do that', 'dgc-wallet' ) );
            }

            if ( ! wp_verify_nonce( $post_data['nonce'], 'dgc_wallet_admin' ) ) {
                wp_send_json_error( __( 'Invalid nonce', 'dgc-wallet' ) );
            }
