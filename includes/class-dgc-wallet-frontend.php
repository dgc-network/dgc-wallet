...
if (!class_exists('dgc_Wallet_Frontend')) {
    ...
    class dgc_Wallet_Frontend {
        ...
        public function add_payment_nav_menu($menu, $args) {
            ...
            ob_start();
            dgc_wallet()->get_template('dgc-wallet-mini.php');
        ...
        public function add_woocommerce_query_vars($query_vars) {
            $query_vars['dgc-wallet'] = get_option('woocommerce_dgc_wallet_endpoint', 'text-domain');
        ...
        public function dgc_wallet_styles() {
            ...
            wp_register_style('dgc-wallet-jquery-ui', '//ajax.googleapis.com/ajax/libs/jqueryui/' . $wp_scripts->registered['jquery-ui-core']->ver . '/themes/smoothness/jquery-ui.css', false, $wp_scripts->registered['jquery-ui-core']->ver, false);
        ...
        /**
         * dgcWallet for WooCommerce menu
         * @param array $items
         * @return array
         */
        public function dgc_wallet_menu_items($items) {
            ...
            $items['dgc-wallet'] = apply_filters('dgc_wallet_account_menu_title', __('dgcWallet', 'text-domain'));
        ...
        public function dgc_wallet_frontend_loaded() {
             ...
             */
            if (isset($_POST['dgc_wallet_transfer_fund']) && apply_filters('dgc_wallet_is_enable_transfer', 'on' === dgc_wallet()->settings_api->get_option('is_enable_payment_transfer', '_wallet_settings_general', 'on'))) {
                $response = $this->do_payment_transfer();
                if (!$response['is_valid']) {
                    wc_add_notice($response['message'], 'error');
                } else {
                    wc_add_notice($response['message']);
                    $location = esc_url( wc_get_account_endpoint_url( get_option( 'woocommerce_dgc_wallet_endpoint', 'text-domain' ) ) );
        ...
        public function is_valid_payment_recharge_amount($amount = 0) {
            ...
            if (isset($_POST['dgc_wallet_topup']) && wp_verify_nonce($_POST['dgc_wallet_topup'], 'dgc_wallet_topup')) {
                if ($min_topup_amount && $amount < $min_topup_amount) {
                    $response = array(
                        'is_valid' => false,
                        'message' => sprintf(__('The minimum amount needed for payment top up is %s', 'text-domain'), wc_price($min_topup_amount, dgc_wallet_wc_price_args()))
                    );
                }
                if ($max_topup_amount && $amount > $max_topup_amount) {
                    $response = array(
                        'is_valid' => false,
                        'message' => sprintf(__('Payment top up amount should be less than %s', 'text-domain'), wc_price($max_topup_amount, dgc_wallet_wc_price_args()))
                    );
                }
                if ($min_topup_amount && $max_topup_amount && ( $amount < $min_topup_amount || $amount > $max_topup_amount )) {
                    $response = array(
                        'is_valid' => false,
                        'message' => sprintf(__('Payment top up amount should be between %s and %s', 'text-domain'), wc_price($min_topup_amount, dgc_wallet_wc_price_args()), wc_price($max_topup_amount, dgc_wallet_wc_price_args()))
                    );
                }
            } else {
                $response = array(
            ...
            return apply_filters('dgc_wallet_is_valid_payment_recharge_amount', $response, $amount);
        }
        ...
        public function do_payment_transfer() {
            ...
            if (isset($_POST['dgc_wallet_transfer']) && wp_verify_nonce($_POST['dgc_wallet_transfer'], 'dgc_wallet_transfer')) {
                ...
                $credit_note = isset($_POST['dgc_wallet_transfer_note']) && !empty($_POST['dgc_wallet_transfer_note']) ? $_POST['dgc_wallet_transfer_note'] : sprintf(__('Payment funds received from %s', 'text-domain'), $current_user_obj->user_email);
                ...
                if ( dgc_wallet()->settings_api->get_option( 'min_transfer_amount', '_wallet_settings_general', 0 ) ) {
                    if ( dgc_wallet()->settings_api->get_option( 'min_transfer_amount', '_wallet_settings_general', 0 ) > $amount) {
                        return array(
                            'is_valid' => false,
                            'message' => sprintf( __('Minimum transfer amount is %s', 'text-domain'), wc_price( dgc_wallet()->settings_api->get_option( 'min_transfer_amount', '_wallet_settings_general', 0 ) ) )
                        );
                    }
                }
                if (!$whom) {
            ...
            if (dgc_wallet()->cashback->calculate_cashback() && !is_payment_rechargeable_cart() && apply_filters('display_cashback_notice_at_woocommerce_page', true)) :
                ...
                <div class="woocommerce-Message woocommerce-Message--info woocommerce-info">
                    ...
                    if (is_user_logged_in()) {
                        echo apply_filters('dgc_wallet_cashback_notice_text', sprintf(__('Upon placing this order a cashback of %s will be credited to your payment.', 'text-domain'), wc_price($cashback_amount, dgc_wallet_wc_price_args())), $cashback_amount);
                    } else {
                        echo apply_filters('dgc_wallet_cashback_notice_text', sprintf(__('Please <a href="%s">log in</a> to avail %s cashback from this order.', 'text-domain'), esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))), wc_price($cashback_amount, dgc_wallet_wc_price_args())), $cashback_amount);
                    }
                    ?>
                </div>
                <?php
        ...
        public function woocommerce_review_order_after_order_total() {
            ...
            wp_enqueue_style('dashicons');
            wp_enqueue_style('dgc-wallet-jquery-ui');
            wp_enqueue_script('jquery-ui-tooltip');
            dgc_wallet()->get_template('dgc-wallet-partial.php');
        }
        ...
        public function woocommerce_available_variation($args, $product_class, $variation){
            ...
            $args['cashback_html'] = wc_price($cashback_amount, dgc_wallet_wc_price_args()). __(' Cashback', 'text-domain');
            return $args;
        }
        ...
        public function display_cashback() {
            ...
            if($cashback_amount){
                echo '<span class="on-dgc-wallet-cashback">' . wc_price($cashback_amount, dgc_wallet_wc_price_args()) . __(' Cashback', 'text-domain') . '</span>';
            } else{
        ...
        public function dgc_wallet_is_enable_transfer($is_enable){
            if('on' != dgc_wallet()->settings_api->get_option( 'is_enable_payment_transfer', '_wallet_settings_general', 'on' )){
                $is_enable = false;
            }
            return $is_enable;
        }
