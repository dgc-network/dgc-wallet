...
class WC_REST_dgc_Wallet_Controller extends WC_REST_Controller {
    ...
    public function register_routes() {
            ...
            array(
                ...
                'args'                => array_merge( $this->get_endpoint_args_for_item_schema( WP_REST_Server::CREATABLE ), array(
                    'type' => array(
                        'required'    => true,
                        'type'        => 'string',
                        'description' => __( 'Wallet transaction type.', 'dgc-wallet' ),
                    ),
                    'amount' => array(
                        'required'    => true,
                        'description' => __( 'Wallet transaction amount.', 'dgc-wallet' ),
                        'type'        => 'number',
                    ),
                    'details' => array(
                        'required'    => false,
                        'description' => __( 'Wallet transaction details.', 'dgc-wallet' ),
                        'type'        => 'string',
                    ),
                ) ),
            ),
        ...
        register_rest_route( $this->namespace, '/' . $this->rest_base . '/balance/(?P<id>[\d]+)', array(
            'args' => array(
                'id' => array(
                    'description' => __( 'Unique identifier for the resource.', 'dgc-wallet' ),
                    'type'        => 'integer',
                ),
            ),
            array(
    ...
    public function get_item( $request ) {
        //get parameters from request
        $params  = $request->get_params();
        $balance = dgc_wallet()->wallet->get_wallet_balance( $params['id'], 'edit' );
    ...
    public function create_item( $request ) {
        ...
        if ( isset( $params['type'] ) && isset( $params['amount'] ) ) {
            ...
            if ( 'credit' === $params['type'] ) {
                $transaction_id = dgc_wallet()->wallet->credit( $params['id'], $params['amount'], $params['details'] );
            } else if ( 'debit' === $params['type'] ) {
                $transaction_id = dgc_wallet()->wallet->debit( $params['id'], $params['amount'], $params['details'] );
            }
    ...
    public function create_item_permissions_check( $request ) {
        if ( ! apply_filters( 'dgc_wallet_rest_check_permissions', current_user_can( 'manage_woocommerce' ), 'create', $request ) ) {
            return new WP_Error( 'woocommerce_rest_cannot_create', __( 'Sorry, you are not allowed to create resources.', 'dgc-wallet' ), array( 'status' => rest_authorization_required_code() ) );
        }
        return true;
    }
