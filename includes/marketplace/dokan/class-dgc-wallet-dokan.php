...
if ( ! class_exists( 'dgc_Wallet_Dokan' ) ) {
    ...
    class dgc_Wallet_Dokan {
         ...
         * @since 1.2.3 
         */
        private static $method_id = 'dgc_wallet';
        ...
        public function __construct() {
            ...
            add_action( 'dgc_wallet_payment_processed', array( $this, 'dgc_wallet_payment_processed' ) );
         ...
         * @throws Exception
         */
        public function dgc_wallet_payment_processed( $order_id ) {
            ...
            if ( ! empty( $all_withdraws ) ) {
                ...
                foreach ( $all_withdraws as $withdraw_data) {
                    ...
                    $data = array(
                        ...
                        'notes'  => sprintf( __( 'Order %d payment Auto paid via Wallet', 'dgc-wallet' ), $withdraw_data['order_id'] ),
                        'ip'     => $IP
                    );

                    $data           = array_merge( $data, $withdraw_data);
                    $transaction_id = dgc_wallet()->wallet->credit( $data['user_id'], $data['amount'], __( 'Commission received for order id ', 'dgc-wallet' ) . $data['order_id'] );
                    if ( $transaction_id ) {
        ...
        public function dokan_product_edit_after_options( $post_id ) {
            ...
            <div class="dokan-cashback-options dokan-edit-row dokan-clearfix" style="margin-bottom: 20px;">
                <div class="dokan-section-heading" data-togglehandler="dokan_cashback_options">
                    <h2><i class="fa fa-cog" aria-hidden="true"></i> <?php _e( 'Cashback Options', 'dgc-wallet' ); ?></h2>
                ...
                <div class="dokan-section-content">
                    ...
                    <div class="dokan-form-group content-half-part">
                        <label for="_cashback_type" class="form-label"><?php _e( 'Cashback type', 'dgc-wallet' ); ?></label>
                        <?php
                        dokan_post_input_box( $post_id, '_cashback_type', array( 'options' => array(
                            'percent' => __( 'Percentage', 'dgc-wallet' ),
                            'fixed'   => __( 'Fixed', 'dgc-wallet' )
                        ) ), 'select' );
                        ?>
                    </div>

                    <div class="dokan-form-group content-half-part">
                        <label for="_cashback_amount" class="form-label"><?php _e( 'Cashback Amount', 'dgc-wallet' ); ?></label>
                        <div class="dokan-input-group">
                            <span class="dokan-input-group-addon"><?php echo get_woocommerce_currency_symbol(); ?></span>
                            <?php dokan_post_input_box( $post_id, '_cashback_amount', array( 'class' => 'dokan-product-sales-price', 'placeholder' => __( '0.00', 'dgc-wallet' ) ), 'number' ); ?>
                        </div>
                    </div>
                </div>
            </div>
        ...
        public function dokan_withdraw_status_updated( $status, $user_id, $row_id ) {
            ...
            if (1 === $status) {
                if ( $wallet_transaction && isset( $wallet_transaction->transaction_id ) ) {
                    ...
                    $transaction_id = dgc_wallet()->wallet->credit( $user_id, $resualt->amount, __( 'Withdrawal request #' . $row_id ) );
