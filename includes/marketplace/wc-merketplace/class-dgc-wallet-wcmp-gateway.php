...
if ( ! class_exists( 'WCMp_Gateway_Wallet' ) && class_exists( 'WCMp_Payment_Gateway' ) ) {
    ...
    class WCMp_Gateway_Wallet extends WCMp_Payment_Gateway {
        ...
        public function __construct() {
            ...
            $this->enabled         = get_wcmp_vendor_settings( 'payment_method_dgc_wallet', 'payment' );
        }
            ...
            if ( $this->validate_request() ) {
                if ( $this->process_wallet_payment() ) {
                    ...
                    if ( $this->transaction_id ) {
                        return array( 'message' => __( 'New transaction has been initiated', 'dgc-wallet' ), 'type' => 'success', 'transaction_id' => $this->transaction_id );
                    }
                } else {
                    return $this->message;
                }
        ...
        public function validate_request() {
            ...
            if ( $this->enabled != 'Enable' ) {
                $this->message[] = array( 'message' => __( 'Invalid payment method', 'dgc-wallet' ), 'type' => 'error' );
                return false;
            }
            if ( $this->transaction_mode != 'admin' ) {
                ...
                if ( $this->get_transaction_total() > $thesold_amount ) {
                    ...
                    $this->message[] = array( 'message' => __( 'Minimum thesold amount to withdrawal commission is ' . $thesold_amount, 'dgc-wallet' ), 'type' => 'error' );
                    return false;
                }
            }
        ...
        private function process_wallet_payment() {
            ...
            $transaction_id  = dgc_wallet()->wallet->credit( $this->vendor->id, $amount_to_pay, __( 'Commission received for commission id ', 'dgc-wallet' ). $for_commissions );
            if ( $transaction_id ) {
