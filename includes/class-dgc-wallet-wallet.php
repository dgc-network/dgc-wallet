...
if ( ! class_exists( 'dgc_Wallet_Wallet' ) ) {

    class dgc_Wallet_Wallet {
        /**
         * WordPress user ID.
         * @var INT 
         */
        ...
        public function wallet_credit_purchase( $order_id ) {
            ...
            $transaction_id = $this->credit( $order->get_customer_id(), $recharge_amount, __( 'Wallet credit through purchase #', 'dgc-wallet' ) . $order->get_order_number() );
            if ( $transaction_id ) {
        ...
        public function wallet_cashback( $order_id ) {
            ...
            if ( apply_filters( 'process_dgc_wallet_general_cashback', !get_post_meta( $order->get_id(), '_general_cashback_transaction_id', true ), $order ) && dgc_wallet()->cashback->calculate_cashback(false, $order->get_id()) ) {
                $transaction_id = $this->credit( $order->get_customer_id(), dgc_wallet()->cashback->calculate_cashback(false, $order->get_id()), __( 'Wallet credit through cashback #', 'dgc-wallet' ) . $order->get_order_number() );
                if ( $transaction_id ) {
            ...
            if ( apply_filters( 'process_dgc_wallet_coupon_cashback', !get_post_meta( $order->get_id(), '_coupon_cashback_transaction_id', true ), $order ) && get_post_meta( $order->get_id(), '_coupon_cashback_amount', true ) ) {
                ...
                if ( $coupon_cashback_amount ) {
                    $transaction_id = $this->credit( $order->get_customer_id(), $coupon_cashback_amount, __( 'Wallet credit through cashback by applying coupon', 'dgc-wallet' ) );
                    if ( $transaction_id ) {
                        update_wallet_transaction_meta( $transaction_id, '_type', 'cashback', $order->get_customer_id() );
        ...
        public function wallet_partial_payment( $order_id ) {
            ...
            if ( $partial_payment_amount && !get_post_meta( $order_id, '_partial_pay_through_wallet_compleate', true ) ) {
                $transaction_id = $this->debit( $order->get_customer_id(), $partial_payment_amount, __( 'For order payment #', 'dgc-wallet' ) . $order->get_order_number() );
                if ( $transaction_id ) {
                    $order->add_order_note(sprintf( __( '%s paid through wallet', 'dgc-wallet' ), wc_price( $partial_payment_amount, dgc_wallet_wc_price_args($order->get_customer_id()) ) ) );
        ...
        public function process_cancelled_order( $order_id ) {
            ...
            $partial_payment_amount = get_order_partial_payment_amount( $order_id );
            if ( $partial_payment_amount && get_post_meta( $order_id, '_partial_pay_through_wallet_compleate', true ) ) {
                $this->credit( $order->get_customer_id(), $partial_payment_amount, sprintf( __( 'Your order with ID #%s has been cancelled and hence your wallet amount has been refunded!', 'dgc-wallet' ), $order->get_order_number() ) );
                $order->add_order_note(sprintf( __( 'Wallet amount %s has been credited to customer upon cancellation', 'dgc-wallet' ), $partial_payment_amount ) );
            ...
            if ( apply_filters( 'dgc_wallet_debit_cashback_upon_cancellation', get_total_order_cashback_amount( $order_id ) ) ) {
                ...
                if ( $total_cashback_amount ) {
                    if ( $this->debit( $order->get_customer_id(), $total_cashback_amount, sprintf( __( 'Cashback for #%s has been debited upon cancellation', 'dgc-wallet' ), $order->get_order_number() ) ) ) {
        ...
        private function record_transaction( $amount, $type, $details ) {
            ...
            //if ( $wpdb->insert( "{$wpdb->base_prefix}dgc_wallet_transactions", apply_filters( 'dgc_wallet_transactions_args', array( 'blog_id' => $GLOBALS['blog_id'], 'user_id' => $this->user_id, 'type' => $type, 'amount' => $amount, 'balance' => $balance, 'currency' => get_woocommerce_currency(), 'details' => $details, 'date' => current_time('mysql') ), array( '%d', '%d', '%s', '%f', '%f', '%s', '%s', '%s' ) ) ) ) {
                ...
                $transaction_id = time();
                $data =  apply_filters( 'dgc_wallet_transactions_args', array( 'deleted' => 0, 'transaction_id' => $transaction_id, 'blog_id' => $GLOBALS['blog_id'], 'user_id' => $this->user_id, 'type' => $type, 'amount' => $amount, 'balance' => $balance, 'currency' => get_woocommerce_currency(), 'details' => $details, 'date' => time() ), array( '%d', '%d', '%s', '%f', '%f', '%s', '%s', '%s' ) ) ;
                $dgc_API_args = array(
