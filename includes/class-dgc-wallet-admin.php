...
if ( ! class_exists( 'dgc_Wallet_Admin' ) ) {
    ...
    class dgc_Wallet_Admin {
        ...
        public function display_actions_table() {
            ...
            settings_errors();
            ?>
            <p><?php _e( 'Integrated wallet actions are listed below. If active those actions will be triggered with respective WordPress hook.', 'dgc-wallet' ); ?></p>
            <table class="wc_emails widefat" cellspacing="0">
                <thead>
                    <tr>
                        <th class="wc-email-settings-table-status"></th>
                        <th class="wc-email-settings-table-name"><?php _e( 'Action', 'dgc-wallet' ); ?></th>
                        <th class="wc-email-settings-table-name"><?php _e( 'Description', 'dgc-wallet' ); ?></th>
                        ...
                        <tr data-gateway_id="<?php echo $action->get_action_id(); ?>">
                            <td>
                                ...
                                if ( $action->is_enabled() ) {
                                    echo '<span class="status-enabled tips" data-tip="' . esc_attr__( 'Enabled', 'dgc-wallet' ) . '">' . esc_html__( 'Yes', 'dgc-wallet' ) . '</span>';
                                } else {
                                    echo '<span class="status-disabled tips" data-tip="' . esc_attr__( 'Disabled', 'dgc-wallet' ) . '">-</span>';
                                }
                                ?>
                            </td>
                            ...
                            <td class="action" width="1%"><a class="button alignright" href="<?php echo esc_url(admin_url( 'admin.php?page=dgc-wallet-actions&action=' . strtolower( $action->id ) ) ); ?>"><?php
                                    if ( $action->is_enabled() ) {
                                        echo __( 'Manage', 'dgc-wallet' );
                                    } else {
                                        echo __( 'Setup', 'dgc-wallet' );
                                    }
                                    ?></a></td>
                        </tr>
                    <?php endforeach; ?>
        ...
        public function admin_scripts() {
            ...
            if (in_array( $screen_id, array( 'product', 'edit-product' ) ) ) {
                ...
                wp_localize_script( 'dgc_wallet_admin_product', 'dgc_wallet_admin_product_param', array( 'product_id' => get_wallet_rechargeable_product()->get_id(), 'is_hidden' => apply_filters( 'dgc_wallet_hide_rechargeable_product', true ) ) );
            }
            if (in_array( $screen_id, array( 'shop_order' ) ) ) {
                ...
                $order_localizer = array(
                    ...
                    'is_refundable' => apply_filters( 'dgc_wallet_is_order_refundable', ( ! is_wallet_rechargeable_order( $order ) && $order->get_payment_method( 'edit' ) != 'wallet' ) && $order->get_customer_id( 'edit' ), $order ),
                    'i18n' => array(
                        'refund' => __( 'Refund', 'dgc-wallet' ),
                        'via_wallet' => __( 'to customer wallet', 'dgc-wallet' )
                    )
                );
        ...
        public function wallet_page() {
            ?>
            <div class="wrap">
                <h2><?php _e( 'Users wallet details', 'dgc-wallet' ); ?></h2>
                <?php do_action('dgc_wallet_before_balance_details_table'); ?>
                <?php $this->balance_details_table->views(); ?>
                <form id="posts-filter" method="post">
                    <?php $this->balance_details_table->search_box( __( 'Search Users', 'dgc-wallet' ), 'search_id' ); ?>
        ...
        public function add_balance_to_user_wallet() {
            ...
            <div class="wrap">
                <?php settings_errors(); ?>
                <h2><?php _e( 'Adjust Balance', 'dgc-wallet' ); ?> <a style="text-decoration: none;" href="<?php echo add_query_arg( array( 'page' => 'dgc-wallet' ), admin_url( 'admin.php' ) ); ?>"><span class="dashicons dashicons-editor-break" style="vertical-align: middle;"></span></a></h2>
                <p>
                    <?php
                    _e( 'Current wallet balance: ', 'dgc-wallet' );
                    echo dgc_wallet()->wallet->get_wallet_balance( $user_id );
                    ?>
                </p>
                <form id="posts-filter" method="post">
                    <table class="form-table">
                        <tbody>
                            <tr>
                                <th scope="row"><label for="balance_amount"><?php echo __( 'Amount', 'dgc-wallet' ) . ' ( ' . get_woocommerce_currency_symbol( $currency) . ' )'; ?></label></th>
                                <td>
                                    <input type="number" step="any" name="balance_amount" class="regular-text" />
                                    <p class="description"><?php _e( 'Enter Amount', 'dgc-wallet' ); ?></p>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row"><label for="payment_type"><?php _e( 'Type', 'dgc-wallet' ); ?></label></th>
                                <td>
                                    <?php $payment_types = apply_filters('dgc_wallet_adjust_balance_payment_type', array('credit' => __( 'Credit', 'dgc-wallet' ), 'debit' => __( 'Debit', 'dgc-wallet' ))); ?>
                                    <select class="regular-text" name="payment_type" id="payment_type">
                                        <?php foreach ($payment_types as $key => $value) : ?>
                                        <option value="<?php echo $key ?>"><?php echo $value; ?></option>
                                        <?php endforeach; ?>
                                    </select>
                                    <p class="description"><?php _e( 'Select payment type', 'dgc-wallet' ); ?></p>
                                </td>
                            </tr>
                            <?php do_action('dgc_wallet_after_payment_type_field') ?>
                            <tr>
                                <th scope="row"><label for="payment_description"><?php _e( 'Description', 'dgc-wallet' ); ?></label></th>
                                <td>
                                    <textarea name="payment_description" class="regular-text"></textarea>
                                    <p class="description"><?php _e( 'Enter Description', 'dgc-wallet' ); ?></p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
        ...
        public function transaction_details_page() {
            ...
            ?>
            <div class="wrap">
                <h2><?php _e( 'Transaction details', 'dgc-wallet' ); ?> <a style="text-decoration: none;" href="<?php echo add_query_arg( array( 'page' => 'dgc-wallet' ), admin_url( 'admin.php' ) ); ?>"><span class="dashicons dashicons-editor-break" style="vertical-align: middle;"></span></a></h2>
                <p><?php _e( 'Current wallet balance: ', 'dgc-wallet' ); echo dgc_wallet()->wallet->get_wallet_balance( $user_id ); ?></p>
        ...
        public function add_dgc_wallet_add_balance_option() {
            if ( $_SERVER['REQUEST_METHOD'] === 'POST' && isset( $_POST['dgc-wallet-admin-adjust-balance'] ) && wp_verify_nonce( $_POST['dgc-wallet-admin-adjust-balance'], 'dgc-wallet-admin-adjust-balance' ) ) {
                ...
                if ( $user_id != NULL && ! empty( $user_id ) && $amount != NULL && ! empty( $amount ) ) {
                    $amount = apply_filters( 'dgc_wallet_addjust_balance_amount', number_format( $amount, wc_get_price_decimals(), '.', '' ), $user_id );
                    if ( 'credit' === $payment_type) {
                        $transaction_id = dgc_wallet()->wallet->credit( $user_id, $amount, $description);
                    } else if ( 'debit' === $payment_type) {
                        $transaction_id = dgc_wallet()->wallet->debit( $user_id, $amount, $description);
                    }
                    if ( !$transaction_id ) {
                        $message = __( 'An error occurred please try again', 'dgc-wallet' );
                    }
                } else {
                    $message = __( 'Please enter amount', 'dgc-wallet' );
                }
                if ( !$transaction_id ) {
        ...
        public function woocommerce_product_data_panels() {
            ...
            <div id="wallet_cashback_product_data" class="panel woocommerce_options_panel">
                ...
                woocommerce_wp_select( array(
                    ...
                    'label' => __( 'Cashback type', 'dgc-wallet' ),
                    'description' => __( 'Select cashback type percentage or fixed', 'dgc-wallet' ),
                    'options' => array( 'percent' => __( 'Percentage', 'dgc-wallet' ), 'fixed' => __( 'Fixed', 'dgc-wallet' ) ),
                ...
                woocommerce_wp_text_input( array(
                    ...
                    'label' => __( 'Cashback Amount', 'dgc-wallet' ),
                    'description' => __( 'Enter cashback amount', 'dgc-wallet' ),
        ...
        public function add_wallet_payment_amount( $order_id ) {
            ...
            if ( $total_cashback_amount = get_total_order_cashback_amount( $order_id ) ) {
                ?>
                <tr>
                    <td class="label"><?php _e( 'Cashback', 'dgc-wallet' ); ?>:</td>
                    <td width="1%"></td>
                    <td class="via-wallet">
        ...
        public function add_coupon_option_for_cashback() {
            woocommerce_wp_checkbox( array(
                ...
                'label' => __( 'Apply as cashback', 'dgc-wallet' ),
                'description' => __( 'Check this box if the coupon should apply as cashback.', 'dgc-wallet' ),
            ) );
        }
        ...
        public function admin_footer_text( $footer_text) {
            ...
            if ( isset( $current_screen->id ) && in_array( $current_screen->id, $dgc_wallet_pages) ) {
                if ( !get_option( 'woocommerce_wallet_admin_footer_text_rated' ) ) {
                    $footer_text = sprintf(
                            __( 'If you like %1$s please leave us a %2$s rating. A huge thanks in advance!', 'dgc-wallet' ), sprintf( '<strong>%s</strong>', esc_html__( 'dgc Wallet for WooCommerce', 'dgc-wallet' ) ), '<a href="https://wordpress.org/support/plugin/dgc-wallet/reviews?rate=5#new-post" target="_blank" class="wc-rating-link" data-rated="' . esc_attr__( 'Thanks :)', 'dgc-wallet' ) . '">&#9733;&#9733;&#9733;&#9733;&#9733;</a>'
                    );
                    wc_enqueue_js( "
				...
				" );
                    ...
                    $footer_text = __( 'Thank you for using dgc Wallet for WooCommerce.', 'dgc-wallet' );
                }
            }
            return $footer_text;
        }
        ...
        public function add_woocommerce_account_endpoint_settings( $settings) {
            $settings_fields = apply_filters( 'dgc_wallet_endpoint_settings_fields', array(
                ...
                array(
                    'title' => __( 'Wallet Transactions', 'dgc-wallet' ),
                    'desc' => __( 'Endpoint for the "My account &rarr; View wallet transactions" page.', 'dgc-wallet' ),
            ...
            $walletendpoint_settings = array(
                array(
                    'title' => __( 'Wallet endpoints', 'dgc-wallet' ),
                    'type' => 'title',
                    'desc' => __( 'Endpoints are appended to your page URLs to handle specific actions on the accounts pages. They should be unique and can be left blank to disable the endpoint.', 'dgc-wallet' ),
        ...
        public function add_product_cat_cashback_field() {
            ...
            <div class="form-field term-display-type-wrap">
                <label for="woo_product_cat_cashback_type"><?php _e( 'Cashback type', 'dgc-wallet' ); ?></label>
                <select name="woo_product_cat_cashback_type" id="woo_product_cat_cashback_type">
                    <option value="percent"><?php _e( 'Percentage', 'dgc-wallet' ); ?></option>
                    <option value="fixed"><?php _e( 'Fixed', 'dgc-wallet' ); ?></option>
                </select>
            </div>
            <div class="form-field term-display-type-wrap">
                <label for="woo_product_cat_cashback_amount"><?php _e( 'Cashback Amount', 'dgc-wallet' ); ?></label>
        ...
        public function edit_product_cat_cashback_field( $term) {
            ...
            ?>
            <tr class="form-field">
                <th scope="row" valign="top"><?php _e( 'Cashback type', 'dgc-wallet' ); ?></th>
                <td>
                    <select name="woo_product_cat_cashback_type" id="woo_product_cat_cashback_type">
                        <option value="percent" <?php selected( $cashback_type, 'percent' ); ?>><?php _e( 'Percentage', 'dgc-wallet' ); ?></option>
                        <option value="fixed" <?php selected( $cashback_type, 'fixed' ); ?>><?php _e( 'Fixed', 'dgc-wallet' ); ?></option>
                    </select>
                </td>
            </tr>
            <tr class="form-field">
                <th scope="row" valign="top"><?php _e( 'Cashback Amount', 'dgc-wallet' ); ?></th>
        ...
        public function manage_users_columns( $columns) {
            if (current_user_can( 'manage_woocommerce' ) ) {
                $columns['current_wallet_balance'] = __( 'Wallet Balance', 'dgc-wallet' );
            }
            return $columns;
        }
        ...
        public function manage_users_custom_column( $value, $column_name, $user_id ) {
            if ( $column_name === 'current_wallet_balance' ) {
                return sprintf( '<a href="%s" title="%s">%s</a>', admin_url( '?page=dgc-wallet-transactions&user_id=' . $user_id ), __( 'View details', 'dgc-wallet' ), dgc_wallet()->wallet->get_wallet_balance( $user_id ) );
            }
            return $value;
        }
        /**
        ...
        public function woocommerce_after_order_fee_item_name_callback( $item_id, $item ){
            ...
            if ( get_post_meta($order_id, '_dgc_wallet_partial_payment_refunded', true) ) {
                echo '<small class="refunded">' . __('Refunded', 'dgc-wallet') . '</small>';
            } else{
                echo '<button type="button" class="button refund-partial-payment">'.__( 'Refund', 'dgc-wallet').'</button>';
            }
        }
        /**
        ...
        public function woocommerce_order_actions($order_actions){
            $order_actions['recalculate_order_cashback'] = __( 'Recalculate order cashback', 'dgc-wallet');
        ...
        public function recalculate_order_cashback($order){
            ...
            if (in_array($order->get_status(), apply_filters('wallet_cashback_order_status', dgc_wallet()->settings_api->get_option('process_cashback_status', '_wallet_settings_credit', array('processing', 'completed'))))) {
                dgc_wallet()->wallet->wallet_cashback($order->get_id());
            }
        }
        ...
        public function show_promotions() {
            ...
            <div class="notice dgc-wallet-promotional-notice">
                ...
                <div class="content">
                    <h2 class=""><?php _e('Obtain Superpowers to get the best out of dgcWallet', 'dgc-wallet'); ?></h2>
                    <p><?php _e('Use superpowers to stand above the crowd. our high-octane add-ons are designed to boost your store wallet features.', 'dgc-wallet'); ?></p>
                    <a href="https://dgc.network/extensions/?utm_source=dgc-wallet-plugin&amp;utm_medium=banner&amp;utm_content=add-on&amp;utm_campaign=extensions" class="button button-primary promo-btn" target="_blank"><?php _e('Learn More', 'dgc-wallet'); ?> →</a>
                </div>
