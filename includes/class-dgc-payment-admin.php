    ...
    class dgc_Payment_Admin {
                    ...
                    <table class="form-table">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php $payment_types = apply_filters('dgc_payment_adjust_balance_payment_type', array('credit' => __( 'Credit', 'text-domain' ), 'debit' => __( 'Debit', 'text-domain' ))); ?>
        ...
        public function admin_footer_text( $footer_text) {
            ...
            if ( isset( $current_screen->id ) && in_array( $current_screen->id, $dgc_payment_pages) ) {
                if ( !get_option( 'woocommerce_payment_admin_footer_text_rated' ) ) {
                    $footer_text = sprintf(
                            __( 'If you like %1$s please leave us a %2$s rating. A huge thanks in advance!', 'text-domain' ), sprintf( '<strong>%s</strong>', esc_html__( 'dgc Payment for WooCommerce', 'text-domain' ) ), '<a href="https://wordpress.org/support/plugin/dgc-payment/reviews?rate=5#new-post" target="_blank" class="wc-rating-link" data-rated="' . esc_attr__( 'Thanks :)', 'text-domain' ) . '">&#9733;&#9733;&#9733;&#9733;&#9733;</a>'
                    );
                    wc_enqueue_js( "
				...
				" );
                    ...
                    $footer_text = __( 'Thank you for using dgc Payment for WooCommerce.', 'text-domain' );
                }
            }
            return $footer_text;
        }
        ...
        public function add_woocommerce_account_endpoint_settings( $settings) {
            $settings_fields = apply_filters( 'dgc_payment_endpoint_settings_fields', array(
                array(
                    'title' => __( 'dgc Payment', 'text-domain' ),
        ...
        public function woocommerce_custom_nav_menu_items( $endpoints) {
            $endpoints[get_option( 'woocommerce_dgc_payment_endpoint', 'text-domain' )] = __( 'dgc Payment', 'text-domain' );
            return $endpoints;
        }
        ...
        public function manage_users_custom_column( $value, $column_name, $user_id ) {
            if ( $column_name === 'current_payment_balance' ) {
                return sprintf( '<a href="%s" title="%s">%s</a>', admin_url( '?page=dgc-payment-transactions&user_id=' . $user_id ), __( 'View details', 'text-domain' ), dgc_payment()->payment->get_payment_balance( $user_id ) );
            }
            return $value;
        }
        ...
        public function show_promotions() {
            ...
            <div class="notice dgc-payment-promotional-notice">
                ...
                <div class="content">
                    <h2 class=""><?php _e('Obtain Superpowers to get the best out of dgcPay', 'text-domain'); ?></h2>
                    ...
                    <a href="https://dgc.network/extensions/?utm_source=dgc-payment-plugin&amp;utm_medium=banner&amp;utm_content=add-on&amp;utm_campaign=extensions" class="button button-primary promo-btn" target="_blank"><?php _e('Learn More', 'text-domain'); ?> →</a>
                </div>
