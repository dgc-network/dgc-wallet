...
$menu_items = apply_filters('dgc_payment_nav_menu_items', array(
    'top_up' => array(
        'title' => apply_filters( 'dgc_payment_account_topup_menu_title', __( 'exchange dgc', 'dgc-payment' ) ),
        'url' => $is_rendred_from_myaccount ? esc_url(wc_get_endpoint_url(get_option('woocommerce_dgc_payment_endpoint', 'dgc-payment'), 'add', wc_get_page_permalink('myaccount'))) : add_query_arg('payment_action', 'add', get_permalink()),
        'icon' => 'dashicons dashicons-plus-alt'
    ),
    'transfer' => array(
        'title' => apply_filters('dgc_payment_account_transfer_amount_menu_title', __('dgc transfer', 'dgc-payment')),
        'url' => $is_rendred_from_myaccount ? esc_url(wc_get_endpoint_url(get_option('woocommerce_dgc_payment_endpoint', 'dgc-payment'), 'transfer', wc_get_page_permalink('myaccount'))) : add_query_arg('payment_action', 'transfer', get_permalink()),
        'icon' => 'dashicons dashicons-randomize'
    ),
    'transaction_details' => array(
        'title' => apply_filters('dgc_payment_account_transaction_menu_title', __('Transactions', 'dgc-payment')),
...
<div class="dgc-payment-my-payment-container">
    <div class="dgc-payment-sidebar">
        <h3 class="dgc-payment-sidebar-heading"><a href="<?php echo $is_rendred_from_myaccount ? esc_url( wc_get_account_endpoint_url( get_option( 'woocommerce_dgc_payment_endpoint', 'dgc-payment' ) ) ) : get_permalink(); ?>"><?php echo apply_filters( 'dgc_payment_account_menu_title', __( 'dgc Payment', 'dgc-payment' ) ); ?></a></h3>
        <ul>
    ...
    <div class="dgc-payment-content">
        <div class="dgc-payment-content-heading">
            <h3 class="dgc-payment-content-h3"><?php _e( 'Balance', 'dgc-payment' ); ?></h3>
            ...
            <?php if ( apply_filters( 'dgc_payment_is_enable_top_up', true ) && ( ( isset( $wp->query_vars['dgc-payment'] ) && 'add' === $wp->query_vars['dgc-payment'] ) || ( isset( $_GET['payment_action'] ) && 'add' === $_GET['payment_action'] ) ) ) { ?>
                <form method="post" action="">
                    <div class="dgc-payment-add-amount">
                        <label for="dgc_payment_balance_to_add"><?php _e( 'Enter amount', 'dgc-payment' ); ?></label>
                        <?php
                        ...
                        <input type="submit" name="woo_add_to_payment" class="woo-add-to-payment" value="<?php _e( 'Add', 'dgc-payment' ); ?>" />
                    </div>
                </form>
            <?php } else if ( apply_filters( 'dgc_payment_is_enable_transfer', 'on' === dgc_payment()->settings_api->get_option( 'is_enable_payment_transfer', '_payment_settings_general', 'on' ) ) && ( ( isset( $wp->query_vars['dgc-payment'] ) && 'transfer' === $wp->query_vars['dgc-payment'] ) || ( isset( $_GET['payment_action'] ) && 'transfer' === $_GET['payment_action'] ) ) ) { ?> 
                <form method="post" action="">
                    <p class="dgc-payment-field-container form-row form-row-wide">
                        <label for="dgc_payment_transfer_user_id"><?php _e( 'Select whom to transfer', 'dgc-payment' ); ?> <?php
                            if ( apply_filters( 'dgc_payment_user_search_exact_match', true ) ) {
                                _e( '(Email)', 'dgc-payment' );
                            }
                            ?></label>
                        <select name="dgc_payment_transfer_user_id" class="dgc-payment-select2" required=""></select>
                    </p>
                    <p class="dgc-payment-field-container form-row form-row-wide">
                        <label for="dgc_payment_transfer_amount"><?php _e( 'Amount', 'dgc-payment' ); ?></label>
                    ...
                    <p class="dgc-payment-field-container form-row form-row-wide">
                        <label for="dgc_payment_transfer_note"><?php _e( 'What\'s this for', 'dgc-payment' ); ?></label>
                    ...
                    <p class="dgc-payment-field-container form-row">
                        ...
                        <input type="submit" class="button" name="dgc_payment_transfer_fund" value="<?php _e( 'Proceed to transfer', 'dgc-payment' ); ?>" />
                    </p>
                </form>
            <?php } ?> 
