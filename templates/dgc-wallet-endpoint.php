 ...
 *
 * This template can be overridden by copying it to yourtheme/dgc-wallet/dgc-wallet-endpoint.php.
 *
...
global $wp;
do_action( 'dgc_wallet_before_my_payment_content' );
...
$menu_items = apply_filters('dgc_wallet_nav_menu_items', array(
    'top_up' => array(
        'title' => apply_filters( 'dgc_wallet_account_topup_menu_title', __( 'Exchange', 'text-domain' ) ),
        'url' => $is_rendred_from_myaccount ? esc_url(wc_get_endpoint_url(get_option('woocommerce_dgc_wallet_endpoint', 'text-domain'), 'add', wc_get_page_permalink('myaccount'))) : add_query_arg('payment_action', 'add', get_permalink()),
        'icon' => 'dashicons dashicons-plus-alt'
    ),
    'transfer' => array(
        'title' => apply_filters('dgc_wallet_account_transfer_amount_menu_title', __('Transfer', 'text-domain')),
        'url' => $is_rendred_from_myaccount ? esc_url(wc_get_endpoint_url(get_option('woocommerce_dgc_wallet_endpoint', 'text-domain'), 'transfer', wc_get_page_permalink('myaccount'))) : add_query_arg('payment_action', 'transfer', get_permalink()),
        'icon' => 'dashicons dashicons-randomize'
    ),
    'transaction_details' => array(
        'title' => apply_filters('dgc_wallet_account_transaction_menu_title', __('Transactions', 'text-domain')),
        'url' => $is_rendred_from_myaccount ? esc_url(wc_get_account_endpoint_url(get_option('woocommerce_dgc_wallet_transactions_endpoint', 'dgc-wallet-transactions'))) : add_query_arg('payment_action', 'view_transactions', get_permalink()),
        'icon' => 'dashicons dashicons-list-view'
    )
), $is_rendred_from_myaccount);
?>

<div class="dgc-wallet-my-payment-container">
    <div class="dgc-wallet-sidebar">
        <h3 class="dgc-wallet-sidebar-heading"><a href="<?php echo $is_rendred_from_myaccount ? esc_url( wc_get_account_endpoint_url( get_option( 'woocommerce_dgc_wallet_endpoint', 'text-domain' ) ) ) : get_permalink(); ?>"><?php echo apply_filters( 'dgc_wallet_account_menu_title', __( 'dgcWallet', 'text-domain' ) ); ?></a></h3>
        <ul>
    ...
    <div class="dgc-wallet-content">
        ...
        <hr/>
        <?php if ( ( isset( $wp->query_vars['dgc-wallet'] ) && ! empty( $wp->query_vars['dgc-wallet'] ) ) || isset( $_GET['payment_action'] ) ) { ?>
            <?php if ( apply_filters( 'dgc_wallet_is_enable_top_up', true ) && ( ( isset( $wp->query_vars['dgc-wallet'] ) && 'add' === $wp->query_vars['dgc-wallet'] ) || ( isset( $_GET['payment_action'] ) && 'add' === $_GET['payment_action'] ) ) ) { ?>
                <form method="post" action="">
                    <div class="dgc-wallet-add-amount">
                        <label for="dgc_wallet_balance_to_add"><?php _e( 'Enter amount', 'text-domain' ); ?></label>
                        <?php
            ...
            <?php } else if ( apply_filters( 'dgc_wallet_is_enable_transfer', 'on' === dgc_wallet()->settings_api->get_option( 'is_enable_payment_transfer', '_wallet_settings_general', 'on' ) ) && ( ( isset( $wp->query_vars['dgc-wallet'] ) && 'transfer' === $wp->query_vars['dgc-wallet'] ) || ( isset( $_GET['payment_action'] ) && 'transfer' === $_GET['payment_action'] ) ) ) { ?> 
                <form method="post" action="">
                    <p class="dgc-wallet-field-container form-row form-row-wide">
                        <label for="dgc_wallet_transfer_user_id"><?php _e( 'Select whom to transfer', 'text-domain' ); ?> <?php
                    ...
                    <p class="dgc-wallet-field-container form-row form-row-wide">
                        <label for="dgc_wallet_transfer_amount"><?php _e( 'Amount', 'text-domain' ); ?></label>
                    ...
                    <p class="dgc-wallet-field-container form-row form-row-wide">
                        <label for="dgc_wallet_transfer_note"><?php _e( 'What\'s this for', 'text-domain' ); ?></label>
                    ...
                    <p class="dgc-wallet-field-container form-row">
                        ...
                        <input type="submit" class="button" name="dgc_wallet_transfer_fund" value="<?php _e( 'Proceed to transfer', 'text-domain' ); ?>" />
                    </p>
                </form>
            <?php } ?> 
...
<?php do_action( 'dgc_wallet_after_my_payment_content' );
